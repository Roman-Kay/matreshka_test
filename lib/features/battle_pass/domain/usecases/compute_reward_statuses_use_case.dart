...
final class ComputeRewardStatusesUseCase {
  ...
  BattlePass call(BattlePass pass) {
    final previousStates = {
      for (final state in pass.playerState.rewardStates) state.rewardId: state,
    };
    ...
    for (final reward in pass.season.instantPremiumRewards) {
      ...
      nextStates.add(
        PlayerRewardState(
          rewardId: reward.id,
          status:
              previous?.status == RewardStatus.received ||
    ...
    return pass.copyWith(
      playerState: pass.playerState.copyWith(rewardStates: nextStates),
    );
  }
}
